...
class TeamAgent(Tool):
    ...
    async def execute(self, action="", team_id="", **kwargs):
        ...
        """
        # Params validation & normalization
    ...
    async def _create_team(self, name="Team", goal="Collaborate on a task", **kwargs):
        ...
        return Response(
            message=self._format_response({
                ...
                "goal": goal,
                "status": "created",
                "next_step": "Step 1: Create all team agent first by using add_agent multiple times to create each specialized agent needed for the task. Only proceed to assigning tasks after all agents are created."
            }),
            break_loop=False
        )
